...
交付文案前核对一件事：**正文里列的视角数量、顺序、名称，必须与图卡完全一致**。19 期（正文写 4 个视角、图卡 5 张）和 20 期（正文写「四个传播学视角」、图卡 5 个视角）都栽在这里。用户对数字很敏感，看到「四个视角」配五张图会直接质疑严谨性。修法：写文案时直接从图卡的 `.point-head` 抄视角名，不要凭记忆重写。
